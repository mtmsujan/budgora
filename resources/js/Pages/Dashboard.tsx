...
export default function Dashboard({ auth, total_income = 0, total_expenses = 0, balance = 0, recent_transactions = [], accounts_summary = [] }: PageProps<DashboardData>) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl p-6 shadow-lg text-white">
                        ...
                        <p className="text-3xl font-bold">{formatCurrency(total_income, currency)}</p>
                    ...
                    <div className="bg-gradient-to-br from-red-500 to-rose-600 rounded-2xl p-6 shadow-lg text-white">
                        ...
                        <p className="text-3xl font-bold">{formatCurrency(total_expenses, currency)}</p>
                        <p className="text-sm opacity-75 mt-2">This month</p>
                    </div>
                                        ...
                                        <div key={transaction.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                                            <div className="flex items-center space-x-4">
                                                ...
                                                <div>
                                                    ...
                                                    {transaction.note && (
                                                        <p className="text-sm text-slate-400 mt-1">{transaction.note}</p>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className={`text-lg font-bold ${isIncome ? 'text-green-600' : isExpense ? 'text-red-600' : 'text-blue-600'}`}>
                                                    {isIncome ? '+' : isExpense ? '-' : ''}{formatCurrency(transaction.amount, currency)}
                                                </p>
                                            </div>
                                        </div>
                                    );
